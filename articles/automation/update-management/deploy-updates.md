...
本文介绍如何计划更新部署并在部署完成后查看执行过程。 可以从所选的 Azure 虚拟机或从所有已配置的计算机的自动化帐户配置更新部署。 

在每个方案下，你创建的部署都以所选计算机为目标；在从自动化帐户创建部署的情况下，你可以以一台或多台计算机为目标。 从 Azure VM 计划更新部署时，步骤与从自动化帐户进行部署的步骤相同，但以下情况例外：
...
若要计划新的更新部署，请执行以下步骤。 下面的步骤适用于所有方案，但在配置部署计划时有细微差别，具体取决于所选的资源（即自动化帐户、Azure VM）。
...
4. 选择更新部署的目标操作系统。
    ...
    > [!NOTE]
    > 如果选择了 Azure VM，则此选项不可用。 系统会自动识别操作系统。
...
15. 此时会回到状态仪表板。 选择“部署计划”以显示已创建的部署计划。 最多会列出 500 个计划。 如果计划超过 500 个，并且你想要查看完整列表，请参阅[软件更新配置 - 列出](https://docs.microsoft.com/rest/api/automation/softwareupdateconfigurations/list) REST API 方法。 指定 API 版本 2019-06-01 或更高版本。
