...
5. 完成脚本后，验证部署。 从自动化帐户中的“混合 Runbook 辅助角色组”页面，在“用户混合 runbook 辅助角色组”选项卡下，会显示新组和成员数。  如果这是现有的组，则成员数会递增。 可从该页上的列表中选择此组，从左侧菜单中选择“混合辅助角色”。 在“混合辅助角色”页上，可以查看列出组的每个成员。
...
2. 将 Log Analytics 代理部署到目标计算机。
    ...
    * 对于非 Azure VM，请使用[将 Windows 计算机连接到 Azure Monitor](../azure-monitor/platform/agent-windows.md) 一文中所述的部署选项安装适用于 Windows 的 Log Analytics 代理。 可以对多台计算机重复此过程，以将多个辅助角色添加到环境。 安装代理后，可将 VM 添加到自动化帐户中的混合 Runbook 辅助角色组。
3. 验证代理是否向工作区报告
