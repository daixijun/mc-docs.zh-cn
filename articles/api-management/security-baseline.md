...
* [了解 Azure 应用程序网关 WAF](../application-gateway/ag-overview.md)
...
* [Azure 应用程序网关上的 Azure Web 应用程序防火墙](../application-gateway/ag-overview.md)
...
为 Azure AD 用户帐户创建诊断设置，并将审核日志和登录日志发送到 Log Analytics 工作区。 在 Log Analytics 中配置所需的警报。

使用 `log-to-eventhub` 策略通过 API 管理配置高级监视，捕获进行安全分析所需的任何其他上下文信息，并将其发送到第三方 SIEM。
