...
可让 Azure SQL 托管实例实现快速轻松的数据库迁移的关键功能之一是对存储在 [Azure 存储](/storage/)上的数据库备份 (`.bak`) 文件进行本机还原。 备份和还原是基于数据库大小的异步操作。 
...
1. 将数据库备份到 Azure blob 存储。 例如，在 [SQL Server Management Studio](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) 中使用[备份到 URL](https://docs.microsoft.com/sql/relational-databases/backup-restore/sql-server-backup-to-url)。 使用 [Microsoft Azure Tools](https://go.microsoft.com/fwlink/?LinkID=324399) 支持早于 SQL Server 2012 SP1 CU2 的数据库。 
