...
1. 若要连接到 VM，请使用 [az vm show](/cli/vm#az-vm-show) 获取 VM 的 IP 地址：
...
1. 若要列出 VM 的恢复点，请使用 [az backup recoverypoint list](/cli/backup/recoverypoint#az-backup-recoverypoint-list) 命令。 在本示例中，我们为在 myRecoveryServicesVault 中受保护的名为 myVM 的 VM 选择最近的恢复点：
...
2. 若要获取将恢复点连接或装载到 VM 的脚本，请使用 [az backup restore files mount-rp](/cli/backup/restore/files#az-backup-restore-files-mount-rp) 命令。 下面的示例可为在 myRecoveryServicesVault 中受保护的名为 myVM 的 VM 获取脚本。
...
3. 若要装载恢复点，请运行脚本。 输入你自己的脚本名称：
    ...
    在脚本运行时，系统会提示你输入密码以访问恢复点。 输入在上一个生成恢复脚本的 [az backup restore files mount-rp](/cli/backup/restore/files#az-backup-restore-files-mount-rp) 命令输出中显示的密码。
...
7. 使用 [az backup restore files unmount-rp](/cli/backup/restore/files#az-backup-restore-files-unmount-rp) 从 VM 卸载恢复点。 下面的示例从 myRecoveryServicesVault 中名为 myVM 的 VM 卸载恢复点。
