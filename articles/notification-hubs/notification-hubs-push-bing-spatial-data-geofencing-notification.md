...
8. 切换到“发布数据源”选项卡，确认列表中显示了该数据源。
    ...
    ![必应地图开发人员中心“管理数据源”页面的屏幕截图，其中“发布数据源”选项卡处于选中状态。](./media/notification-hubs-geofence/bing-maps-published-data.png)
...
10. 现在已满足数据源的所有要求。 要获取有关 API 调用的请求 URL 的详细信息，请在必应地图开发人员中心内依次选择“数据源”、“数据源信息”。
    ...
    ![标准 JSON 响应的屏幕截图。](./media/notification-hubs-geofence/bing-maps-json.png)

    仅当位置点确实位于指定边界内时，才出现此响应。 如果不在边界内，将收到空白的 **results** 桶：

    ![Results 桶为空的 JSON 响应的屏幕截图。](./media/notification-hubs-geofence/bing-maps-nores.png)

## <a name="set-up-the-uwp-application"></a>设置 UWP 应用程序

1. 在 Visual Studio 中，启动“空白应用(通用 Windows)”类型的新项目。

    ![Visual Studio“新建项目”对话框的屏幕截图，其中突出显示了“空白应用(通用 Windows Visual C#)”选项。](./media/notification-hubs-geofence/notification-hubs-create-blank-app.png)
...
4. 在解决方案中创建名为 `Core` 的新文件夹，并在其中添加名为 `LocationHelper.cs` 的新文件：
    ...
    ![解决方案资源管理器的屏幕截图，其中突出显示了新的 Core 文件夹。](./media/notification-hubs-geofence/vs-location-helper.png)
    ...
    若想详细了解如何在 UWP 应用中获取用户的位置，请参阅[获取用户的位置](https://docs.microsoft.com/windows/uwp/maps-and-location/get-location)。
...
6. 运行应用程序，并允许它访问你的位置。
    ...
    ![“是否允许 NotificationHubs.Geofence 访问你的位置?”对话框的屏幕截图。](./media/notification-hubs-geofence/notification-hubs-location-access.png)
7. 启动应用程序后，应该可以在“输出”窗口中看到坐标：

    ![显示坐标的输出窗口的屏幕截图。](./media/notification-hubs-geofence/notification-hubs-location-output.png)
...
3. 将 `AppBackend` 项目设置为“启始项目”并将它启动。
    ...
    ![解决方案右键单击菜单的屏幕截图，其中突出显示了“设为启动项目”选项。](./media/notification-hubs-geofence/vs-startup-project.png)
...
1. 首先，需要确保地域隔离区是动态的。 需要对必应 API 进行一些额外的处理，才能在现有数据源内上传新边界。 有关详细信息，请参阅[必应空间数据服务 API 文档](https://docs.microsoft.com/bingmaps/spatial-data-services/)。
