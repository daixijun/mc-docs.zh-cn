...
ms.openlocfilehash: 2cbc6ce27cf8a0821792bdb1fc24c4540441ff65
ms.sourcegitcommit: 6b499ff4361491965d02bd8bf8dde9c87c54a9f5
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/06/2020
ms.locfileid: "94327730"
---
...
1. 如果查询语法正确，将以图表形式显示查询的历史数据，同时显示用于调整图表时间段（从过去六小时到过去一周）的选项。
    ...
    如果查询结果包含汇总数据或[项目](https://docs.microsoft.com/azure/kusto/query/projectoperator)特定列（不包含时间列），则图表将显示单个值。
...
1. 日志警报可以基于两种类型的[“度量值”](alerts-unified-log.md#measure)：
    1. **结果数** - 查询返回的记录的计数。
    1. **指标度量** - 使用通过按选择的表达式和 [bin()](https://docs.microsoft.com/azure/kusto/query/binfunction) 选择分组的汇总计算得出的聚合值。 例如：
...
1. 对于指标度量警报逻辑，可以选择使用“聚合对象”选项来指定[如何按维度拆分警报](alerts-unified-log.md#split-by-alert-dimensions)。 行分组表达式必须唯一且已排序。
    ...
    > [!NOTE]
    > 按警报维度拆分仅适用于当前的 scheduledQueryRules API。 仅在 API 版本 `2020-05-01-preview` 及更高版本中支持大规模的以资源为中心的警报。
...
1. 在[门户](https://portal.azure.cn/)中，选择“Monitor”，然后选择“警报” 。
...
1. 在[门户](https://portal.azure.cn/)中，选择相关资源或“Monitor”服务。 然后，在“Monitor”部分中选择“警报”。
...
下面列出的 PowerShell cmdlet 可用于使用[计划查询规则 API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/) 来管理规则。
...
> [!NOTE]
> ScheduledQueryRules PowerShell cmdlets 只能管理在当前[计划查询规则 API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/) 中创建的规则。
...
前面几个部分介绍了如何使用 Azure 门户创建、查看和管理日志预警规则。 本部分将介绍如何使用跨平台 [Azure CLI](/cli/get-started-with-azure-cli) 实现相同的结果。
