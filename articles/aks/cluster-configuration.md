...
ms.author: v-yeche
author: rockboyfor
ms.openlocfilehash: 730ad181a2861cde16bd044e4bfda4787394de65
ms.sourcegitcommit: bfdca6e8227c3d49a42857d7540673573a121ab8
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/21/2021
ms.locfileid: "98626335"
---
...
```azurecli
az aks create --name myAKSCluster --resource-group myResourceGroup --kubernetes-version 1.18.10
```
...
```azurecli
az aks upgrade --name myAKSCluster --resource-group myResourceGroup --kubernetes-version 1.18.10
```

如果只想升级一个节点池，请执行以下命令：

```azurecli
az aks nodepool upgrade -name ubuntu1804 --cluster-name myAKSCluster --resource-group myResourceGroup --kubernetes-version 1.18.10
```

### <a name="test-aks-ubuntu-1804-generally-available-on-existing-clusters"></a>测试现有群集上的 AKS Ubuntu 18.04 正式发布版
...
```azurecli
az aks upgrade --name myAKSCluster --resource-group myResourceGroup --kubernetes-version 1.18.10 --control-plane-only

az aks nodepool add --name ubuntu1804 --cluster-name myAKSCluster --resource-group myResourceGroup --kubernetes-version 1.18.10
```
...
:::image type="content" source="media/cluster-configuration/docker-cri.png" alt-text="Docker CRI 1":::
...
:::image type="content" source="media/cluster-configuration/containerd-cri.png" alt-text="Docker CRI 2":::
...
* `Containerd` 使用标准化 `cri` 日志记录格式（不同于你当前从 Docker 的 JSON 驱动程序获取的内容）设置日志记录。 日志记录解决方案需要支持 `cri` 日志记录格式（例如，[用于容器的 Azure Monitor](../azure-monitor/insights/container-insights-enable-new-cluster.md)）
...
<!-- Update_Description: new article about cluster configuration -->
<!--NEW.date: 01/25/2021-->