...
<a name="azure-rbac-for-kubernetes-authorization-preview"></a>
...
:::image type="content" source="media/concepts-identity/azure-rbac-k8s-authz-flow.png" alt-text="使用 Azure RBAC 执行 Kubernetes 授权流":::

如上图所示，使用 Azure RBAC 集成时，对 Kubernetes API 的所有请求都将遵循 [Azure Active Directory 集成部分](#azure-active-directory-integration)所述的身份验证流。 
...
1. 运行 `az cloud set -n AzureChinaClou` 和 `az login` 向 Azure 进行身份验证。
