...
1. 在 [Azure 门户](https://portal.azure.cn/)中，导航到你的服务总线命名空间。 选择左侧菜单上的“访问控制(标识和访问管理)”，显示命名空间的访问控制设置  。 如果需要创建服务总线命名空间，请按此文中的说明操作：[创建服务总线消息传送命名空间](service-bus-create-namespace-portal.md)。
    ...
    :::image type="content" source="./media/authenticate-application/select-access-control-menu.png" alt-text="在左侧菜单中选择“访问控制”":::
...
1. 在“添加角色分配”  页上，执行以下步骤：
    ...
    4. 分配有该角色的标识列出在该角色下。 例如，下图显示 Azure-users 充当“Azure 服务总线数据所有者”角色。 
        ...
        :::image type="content" source="./media/authenticate-application/user-in-list.png" alt-text="列表中的用户":::
...
将 Azure AD 与服务总线配合使用的主要优势之一在于，不再需要在代码中存储凭据。 可以改为从 Azure 标识平台请求 OAuth 2.0 访问令牌。 Azure AD 对运行应用程序的安全主体（用户、组或服务主体）进行身份验证。 如果身份验证成功，Azure AD 会将访问令牌返回应用程序，应用程序可随之使用访问令牌对 Azure 服务总线请求授权。
...
:::image type="content" source="./media/authenticate-application/app-registrations-register.png" alt-text="注册应用程序":::
...
:::image type="content" source="./media/authenticate-application/application-id.png" alt-text="已注册的应用程序的应用程序 ID":::
...
1. 在“客户端机密”下，选择“新建客户端机密”以创建新的机密。
    ...
    :::image type="content" source="./media/authenticate-application/new-client-secret-button.png" alt-text="新建客户端机密 - 按钮":::
1. 提供机密说明，并选择所需的过期时间间隔，然后选择“添加”。

    :::image type="content" source="./media/authenticate-application/add-client-secret-page.png" alt-text="“添加客户端机密”页":::
1. 请马上将新机密的值复制到安全位置。 填充值只会显示一次。

    :::image type="content" source="./media/authenticate-application/client-secret.png" alt-text="客户端机密":::
