---
title: Azure 安全中心中所有 Azure Defender 警报的引用表
...
ms.openlocfilehash: 397040bf219c40ca931509015c3e9bcb76b4b0ea
ms.sourcegitcommit: d30cf549af09446944d98e4bd274f52219e90583
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/15/2020
ms.locfileid: "94638102"
---
# <a name="security-alerts---a-reference-guide"></a>安全警报 - 参考指南

本文列出可能会从 Azure Defender 中获取的安全警报。 环境中显示的警报取决于要保护的资源和服务以及自定义的配置。

若要了解如何响应这些警报，请参阅[在 Azure 安全中心管理和响应安全警报](security-center-managing-and-responding-alerts.md)。

若要了解如何导出警报（和建议），请参阅[连续导出安全中心数据](continuous-export.md)。
...
|**检测到无文件攻击工具包**|指定进程的内存包含无文件攻击工具包：[工具包名称]。 无文件攻击工具包使用的方法可最大程度减少或消除磁盘上恶意软件的痕迹，并大幅降低基于磁盘的恶意软件扫描解决方案检测到它们的几率。 具体行为包括：<br>1) 众所周知的工具包和加密挖掘软件。<br>2) Shellcode，这是一小段代码，通常在利用软件漏洞时用作有效负载。<br>3) 在进程内存中注入了恶意可执行文件。|防御规避|中型|
...
|警报（警报类型）|说明|Intent<br>（[了解详细信息](#intentions)）|严重性|
|----|----|:----:|--|
...
|**异常终止**<br>(VM_AbnormalDaemonTermination)|分析 %{Compromised Host} 上的主机数据时检测到守护进程异常终止。|利用|低|
...
|**突发日志删除可能指示存在攻击者的操作 [多次出现)**|[Machine] 上的主机数据分析检测到大量系统日志文件被删除。 攻击者经常会执行此操作以规避防御。今天此行为在下列计算机上出现了 100 次以上：[计算机名称]|防御规避|低|
|**突发日志删除可能指示存在攻击者的操作**|[Machine] 上的主机数据分析检测到大量系统日志文件被删除。 攻击者经常会执行此操作以规避防御。|防御规避|低|
...
|**在服务帐户中运行的进程意外地成为根进程。**<br>(VM_SuspectProcessAccountPrivilegeCombo)|在服务帐户 %{User Name} 中运行的进程“%{Suspicious Process}”意外地成为根进程。|特权提升|中型|
...
|**检测到与安全性相关的进程终止**<br>(VM_SuspectProcessTermination)|分析 %{Compromised Host} 上的主机数据时检测到有人试图终止与主机上的安全监视相关的进程。 攻击者通常会在入侵后尝试使用预定义的脚本来终止此类进程。|持久性、规避防御|中型|
...
|**主机配置的可疑枚举**|[Machine] 上的主机数据分析检测到可能存在恶意主机枚举。 有时，恶意操作者会在最初入侵计算机时运行这些命令。|-|中型|
|**可疑的文件时间戳修改操作**|主机数据分析检测到可疑的时间戳修改操作。 攻击者通常将时间戳从现有的合法文件复制到新工具，以绕开对这些新放置的文件的检测。|持久性/防御规避|低|
|**shell 中的第一个命令可疑**<br>(VM_SuspectInitialShellCommand)|分析 %{Compromised Host} 上的主机数据时检测到在 shell 中执行的第一个命令可疑。 这可能指示存在恶意活动。|利用|低|
...
|||||
<!--Not available in MC: Alerts for Azure App Service-->
...
| **异常的导出位置**                     | 有人已将大量数据从你的 SQL Server“{SQL Server name}”提取到异常位置。 | 外泄                       | 高     |
|                                                 |                                                                                                                                                                                                                                                                                                                                                                                                                                             |                                    |          |
<!--Not available in MC: Alerts for Azure Storage-->


<!--Not available in MC: Alerts for Azure Key Vault-->


<!--Not available in MC: Azure DDoS Protection-->
