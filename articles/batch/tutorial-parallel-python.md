...
<!--MOONCAKE: CORRECT ON APPEND _STORAG_ENDPOINT_SUFFIX-->
...
_STORAG_ENDPOINT_SUFFIX = 'core.chinacloudapi.cn'
```
...
> [!NOTE]
> 在运行 `python batch_python_tutorial_ffmpeg.py` 脚本之前，请在调用名为 `azureblob.BlockBlobService` 的函数时追加 `endpoint_suffix=config._STORAG_ENDPOINT_SUFFIX` 的参数。
> 
> 更新后的响应脚本应类似于以下代码：
> ```python
...
>   endpoint_suffix=config._STORAG_ENDPOINT_SUFFIX)
> ```
...
:::image type="content" source="./media/tutorial-parallel-python/pool.png" alt-text="池热度地图":::
...
<!--MOONCAKE CUSTOMIZE REMOVE LOW-PRIORITY ON 08/21/2020-->

节点数和 VM 大小使用定义的常数进行设置。 Batch 支持专用节点。 专用节点为池保留。  默认情况下，此示例创建的池只包含 5 个大小为 Standard_A1_v2 的专用节点。 

<!--Not Available on , and you can use either or both in your pools-->
<!--Not Available on Low-priority nodes are offered at a reduced price from surplus VM capacity in Azure. Low-priority nodes become unavailable if Azure does not have enough capacity.>
<!--Not Available on and [low-priority nodes](batch-low-pri-vms.md)-->
<!--MOONCAKE CUSTOMIZE REMOVE LOW-PRIORITY ON 08/21/2020-->
