...
- 为备份 IO 预配更多 IOP
- 或者，如果基础存储基础结构在你的首选 [Azure 区域](/mysql/concepts-pricing-tiers#storage)中可用，则可以迁移到支持存储容量最大达 16 TB 的常规用途存储。 支持最大 16 TB 存储的常规用途存储不产生额外的费用。 若要获得有关迁移到 16 TB 存储的帮助，请从 Azure 门户创建支持工单。 
