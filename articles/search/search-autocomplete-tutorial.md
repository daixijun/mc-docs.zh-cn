...
`InitSearch` 方法在 Azure 认知搜索服务中创建经过身份验证的 HTTP 索引客户端。 [SuggestOptions](https://docs.microsoft.com/dotnetapi/azure.search.documents.suggestoptions) 类的属性决定了要搜索并要在结果中返回的字段、匹配项的数量以及是否使用模糊匹配。 
