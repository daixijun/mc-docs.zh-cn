...
1. 在与其他订阅者共享的免费层上[创建一个搜索服务](search-create-service-portal.md)， https://www.azure.cn/pricing/details/m/pricing/details/search/) 可搜索仅供你的服务使用的专用资源。 所有快速入门和教程都可以通过免费服务完成。

1. 使用门户 [REST API](https://docs.microsoft.com/rest/api/searchservice/create-index) [创建搜索索引](search-what-is-an-index.md)。 [.NET SDK](search-howto-dotnet-sdk.md) 或其他 SDK。 索引架构决定了可搜索内容的结构。
