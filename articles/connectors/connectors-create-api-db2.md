...
ms.author: v-yeche
tags: connectors
ms.openlocfilehash: fef98d7e0dba8891b2cfb618896eab78ed68b451
ms.sourcegitcommit: 8f438bc90075645d175d6a7f43765b20287b503b
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/10/2020
ms.locfileid: "97004337"
---
<!--Verified successfully-->
# <a name="access-and-manage-ibm-db2-resources-by-using-azure-logic-apps"></a>使用 Azure 逻辑应用访问和管理 IBM DB2 资源

使用 [Azure 逻辑应用](../logic-apps/logic-apps-overview.md)和 [IBM DB2 连接器](https://docs.microsoft.com/connectors/db2/)，可以基于 DB2 数据库中存储的资源创建自动化的任务和工作流。 工作流可以连接到数据库中的资源、读取和列出数据库表、添加行、更改行、删除行，以及执行其他操作。 可在逻辑应用中包含操作，用于从数据库获取响应，并使输出可供其他操作使用。
...
1. 在搜索框中，输入“db2”作为筛选器。 对于本示例，请在操作列表中选择以下操作：“获取表(预览)”
    ...
    :::image type="content" source="./media/connectors-create-api-db2/select-db2-action.png" alt-text="选择操作":::
...
| **服务器** | 是 | DB2 服务器的地址或冒号分隔的别名端口号，例如“myDB2server.chinacloudapp.cn:50000” <p><p>**注意**：此值是表示 TCP/IP 地址或别名的字符串，采用 IPv4 或 IPv6 格式，后接冒号和 TCP/IP 端口号。 |
...
:::image type="content" source="./media/connectors-create-api-db2/create-db2-cloud-connection.png" alt-text="基于云的数据库的连接详细信息":::
...
:::image type="content" source="./media/connectors-create-api-db2/create-db2-on-premises-connection.png" alt-text="本地数据库的连接详细信息":::
...
1. 在“运行历史记录”部分中的“摘要”下，选择最近的运行，即列表中的第一项。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/run-history.png" alt-text="查看运行历史记录":::

1. 在“逻辑应用运行”下，现在可以查看逻辑应用中每个步骤的状态、输入和输出。
展开“获取表”操作。

    :::image type="content" source="./media/connectors-create-api-db2/expand-action-step.png" alt-text="展开操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
   ...
   :::image type="content" source="./media/connectors-create-api-db2/db2-connector-get-tables-outputs.png" alt-text="查看输出表":::
...
1. 如果以前从未在逻辑应用中用过 DB2 操作，请查看[添加 DB2 操作 - 获取表](#add-db2-action)部分中的步骤，但应该添加“获取一行”操作，然后返回此处以继续。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-get-row-action.png" alt-text="“获取一行”操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-connector-get-row-outputs.png" alt-text="查看输出行":::
...
1. 如果以前从未在逻辑应用中用过 DB2 操作，请查看[添加 DB2 操作 - 获取表](#add-db2-action)部分中的步骤，但应该添加“获取多行”操作，然后返回此处以继续。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-get-rows-action.png" alt-text="“获取多行”操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-connector-get-rows-outputs.png" alt-text="查看输出行":::
...
1. 如果以前从未在逻辑应用中用过 DB2 操作，请查看[添加 DB2 操作 - 获取表](#add-db2-action)部分中的步骤，但应该添加“插入行”操作，然后返回此处以继续。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-insert-row-action.png" alt-text="“插入行”操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-connector-insert-row-outputs.png" alt-text="查看输出和插入的行":::
...
1. 如果以前从未在逻辑应用中用过 DB2 操作，请查看[添加 DB2 操作 - 获取表](#add-db2-action)部分中的步骤，但应该添加“更新行”操作，然后返回此处以继续。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-update-row-action.png" alt-text="“更新行”操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-connector-update-row-outputs.png" alt-text="查看输出和更新的行":::
...
1. 如果以前从未在逻辑应用中用过 DB2 操作，请查看[添加 DB2 操作 - 获取表](#add-db2-action)部分中的步骤，但应该添加“删除行”操作，然后返回此处以继续。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-delete-row-action.png" alt-text="“删除行”操作":::
...
1. 若要查看输出，请选择“显示原始输出”。
    ...
    :::image type="content" source="./media/connectors-create-api-db2/db2-connector-delete-row-outputs.png" alt-text="查看不包括已删除行的输出":::

## <a name="connector-reference"></a>连接器参考

有关此连接器的更多技术详细信息，例如触发器、操作和限制（如此连接器的 Swagger 文件所述），请参阅[连接器的参考页](https://docs.microsoft.com/connectors/db2/)。
...
<!--Not Available on [ISE message limits](../logic-apps/logic-apps-limits-and-config.md#message-size-limits)-->

## <a name="next-steps"></a>后续步骤

* 了解其他[逻辑应用连接器](../connectors/apis-list.md)

<!-- Update_Description: new article about connectors create api db2 -->
<!--NEW.date: 12/14/2020-->