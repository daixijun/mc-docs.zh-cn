---
title: 集成和管理安全操作与 Azure Graph 安全性
description: 通过使用 Azure Graph 安全性和 Azure 逻辑应用来提升应用的威胁防护、检测和响应能力
services: logic-apps
...
逻辑应用的工作流可使用操作，以从 Azure Graph 安全性连接器获取响应，并提供该输出给工作流中的其他操作使用。 另外，还可以让工作流中的其他操作使用 Azure Graph 安全性连接器操作的输出。 例如，如果通过 Azure Graph 安全性连接器获取非常严重的警报，可使用 Outlook 连接器在电子邮件中发送这些警报。 
...
* 要访问 Azure Graph 安全性实体（如警报）的逻辑应用。 若要使用 Microsoft Graph 安全性触发器，需要一个空白逻辑应用。 若要使用 Azure Graph 安全性操作，需要一个由适用于你的场景的触发器开始的逻辑应用。
...
下面更详细地介绍了如何使用各种 Azure Graph 安全性连接器操作。

### <a name="manage-alerts"></a>管理警报

若要筛选、排序或获取最新结果，只需提供 [Azure Graph支持的 ODATA 查询参数](https://docs.microsoft.com/graph/query-parameters)。 请勿指定完整的基 URL 或 HTTP 操作，例如 `https://graph.chinacloudapi.cn/v1.0/security/alerts` 或 `GET`/`PATCH` 操作。 下面的具体示例展示了在要获取非常严重的警报列表时“获取警报”操作的参数：
...
若要筛选、排序或获取最新结果，只需提供 [Azure Graph支持的 ODATA 查询参数]https://docs.microsoft.com/graph/query-parameters)。 请勿指定完整的基 URL 或 HTTP 操作，例如 `https://graph.chinacloudapi.cn/beta/security/tiIndicators` 或 `GET`/`PATCH` 操作。 下面的具体示例展示了在要获取具有 `DDoS` 威胁类型的列表时“获取 tiIndicator”操作的参数：

`Filter threat intelligence indicator value as threatType eq 'DDoS'`

若要详细了解可以与此连接器一起使用的查询，请参阅 [Azure Graph 安全性威胁情报指示器参考文档中的“可选查询参数”]https://docs.microsoft.com/graph/api/tiindicators-list) 。 若要使用此连接器增强体验，请详细了解[连接器支持的架构属性威胁情报指示器]https://docs.microsoft.com/graph/api/resources/tiindicator) 。
