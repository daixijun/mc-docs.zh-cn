...
description: 了解如何使用 Azure CLI 通过 Azure 基于角色的访问控制 (Azure RBAC) 向 Azure Active Directory 安全主体分配权限。 Azure 存储支持通过 Azure AD 使用内置的和 Azure 自定义的角色进行身份验证。
services: storage
author: WenJason
ms.service: storage
ms.topic: how-to
...
本文介绍如何使用 Azure CLI 列出内置的 Azure 角色并将其分配给用户。 若要详细了解如何使用 Azure CLI，请参阅 [Azure 命令行界面 (CLI)](/cli/)。
...
若要通过 Azure CLI 列出可用的内置 Azure 角色，请使用 [az role definition list](/cli/role/definition#az-role-definition-list) 命令：
