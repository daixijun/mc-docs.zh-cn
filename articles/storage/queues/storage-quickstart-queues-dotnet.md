...
ms.openlocfilehash: 83c7c1d5b0e6360439238018aa2801f1d286d7b8
ms.sourcegitcommit: f086abe8bd2770ed10a4842fa0c78b68dbcdf771
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/13/2021
ms.locfileid: "98163089"
---
...
- [示例](../common/storage-samples-dotnet.md?toc=%2fstorage%2fqueues%2ftoc.json#queue-samples)
...
- [`QueueServiceClient`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueserviceclient)：可以通过 `QueueServiceClient` 管理存储帐户中的所有队列。
- [`QueueClient`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient)：可以通过 `QueueClient` 类管理和操作单个队列及其消息。
- [`QueueMessage`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.models.queuemessage)：`QueueMessage` 类表示在队列上调用 [`ReceiveMessages`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.receivemessages) 时返回的单个对象。
...
创建 [`QueueClient`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient) 类的实例。 然后，通过调用 [`CreateAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.createasync) 方法，在存储帐户中创建队列。
...
以下代码片段通过调用 [`SendMessageAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.sendmessageasync) 方法，以异步方式将消息添加到队列。 它还保存从 `SendMessageAsync` 调用返回的 [`SendReceipt`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.models.sendreceipt)。 收据用于在稍后的程序中更新消息。
...
通过调用 [`PeekMessagesAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.peekmessagesasync) 方法，查看队列中的消息。 此方法从队列前面检索一条或多条消息，但不更改消息的可见性。
...
通过调用 [`UpdateMessageAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.updatemessageasync) 方法来更新消息的内容。 此方法可以更改消息的可见性超时和内容。 消息内容必须是最大为 64 KB 的 UTF-8 编码字符串。 除了该消息的新内容，还会传入代码中之前保存的 `SendReceipt` 中的值。 `SendReceipt` 值标识要更新的消息。
...
通过调用 [`ReceiveMessagesAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.receivemessagesasync) 方法，下载以前添加的消息。
...
以下代码使用 [`DeleteAsync`](https://docs.microsoft.com/dotnet/api/azure.storage.queues.queueclient.deleteasync) 方法来删除队列，以便清除该应用所创建的资源。
