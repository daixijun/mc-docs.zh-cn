...
ms.openlocfilehash: 0e73518a8e11f75cd66549728b2d6488bbaf63a2
ms.sourcegitcommit: f086abe8bd2770ed10a4842fa0c78b68dbcdf771
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/13/2021
ms.locfileid: "98163147"
---
...
- [示例](../common/storage-samples-javascript.md?toc=%2fstorage%2fqueues%2ftoc.json#queue-samples)
...
- [`QueueMessage`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queuemessage)：`QueueMessage` 类表示在队列上调用 [`ReceiveMessages`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#receivemessages-queuereceivemessageoptions-) 时返回的单个对象。
...
创建 [`QueueClient`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient) 类的实例。 然后，通过调用 [`create`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#create-queuecreateoptions-) 方法，在存储帐户中创建队列。
...
通过调用 [`peekMessages`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#peekmessages-queuepeekmessagesoptions-) 方法，查看队列中的消息。 此方法从队列前面检索一条或多条消息，但不更改消息的可见性。
...
通过调用 [`receiveMessages`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#receivemessages-queuereceivemessageoptions-) 方法，下载以前添加的消息。 在 `numberOfMessages` 字段中，传入要为此调用接收的最大消息数。
...
通过调用 [`deleteMessage`](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#deletemessage-string--string--queuedeletemessageoptions-) 方法删除消息。 未显式删除的任何消息都将在队列中再次变为可见，以方便再一次进行处理。
