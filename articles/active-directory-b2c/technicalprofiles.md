...
ms.author: v-junlch
ms.subservice: B2C
ms.openlocfilehash: 135f481acdd9fc6e413fca4ca81e160b496370e1
ms.sourcegitcommit: 292892336fc77da4d98d0a78d4627855576922c5
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/19/2021
ms.locfileid: "98570625"
---
...
Azure AD B2C 以策略密钥的形式存储机密和证书，从而与它所集成的服务建立信任。 在执行技术配置文件过程中，Azure AD B2C 从 Azure AD B2C 策略密钥中检索加密密钥，然后使用这些密钥建立信任、加密或签名令牌。 这些信任包括：

- OAuth1、OAuth2 和 [SAML](saml-identity-provider-technical-profile.md#cryptographic-keys) 标识提供者的联合身份验证
...
DisplayClaims 元素包含由[自断言的技术配置文件](self-asserted-technical-profile.md)定义的声明列表，这些声明将呈现在屏幕上以从用户处收集数据。 在显示声明集合中，可以包含对已创建的[声明类型](claimsschema.md)或 [DisplayControl](display-controls.md) 的引用。 
...
技术配置文件可以包括另一个技术配置文件，用以更改设置或添加新功能。 IncludeTechnicalProfile 元素是对通用技术配置文件的引用，可从中派生技术配置文件。 若要降低策略元素的冗余性和复杂性，当你具有共享核心元素的多个技术配置文件时，请使用包含。 结合使用通用技术配置文件与一组通用配置，以及包含通用技术配置文件的特定任务技术配置文件。 例如，假设你有一个 [REST API 技术配置文件](restful-technical-profile.md)，该配置文件具有单个终结点，其中你需要针对不同的方案发送不同的声明集。 创建具有共享功能的通用技术配置文件，例如 REST API 终结点 URI、元数据、身份验证类型和加密密钥。 然后创建包含通用技术配置文件的特定任务技术配置文件，添加输入声明、输出声明，或覆盖与该技术配置文件相关的 REST API 终结点 URI。
...
<ClaimsProvider>
  ...
  <TechnicalProfiles>
    <TechnicalProfile Id="REST-API-Commom">
      ...
      <Protocol Name="Proprietary" Handler="Web.TPEngine.Providers.RestfulProvider, Web.TPEngine, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null" />
      <Metadata>
        <Item Key="ServiceUrl">https://your-app-name.chinacloudsites.cn/api/identity</Item>
    ...
    <TechnicalProfile Id="REST-UpdateProfile">
       <Metadata>
        <Item Key="ServiceUrl">https://your-app-name.chinacloudsites.cn/api/identity/update</Item>
      </Metadata>
