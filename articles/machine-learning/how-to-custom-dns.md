...
将 Azure 机器学习和虚拟网络一起使用时，有[几种方法可用于处理 DNS 名称解析](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)。 默认情况下，Azure 会自动处理工作区和专用终结点的名称解析。 如果改为使用自己的自定义 DNS server_，需要手动为工作区创建 DNS 条目。
