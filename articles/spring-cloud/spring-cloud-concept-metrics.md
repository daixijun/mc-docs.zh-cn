...
>| 名称 | Spring Actuator 指标名称 | 计价单位 | 详细信息 |
>|----|----|----|------------|
>| system.cpu.usage | system.cpu.usage | 百分比 | 整个系统最近的 CPU 使用率。 此值为 [0.0,1.0] 区间中的一个双精度值。 如果值为 0.0，则表示在所观察的最近一段时间内所有 CPU 都处于空闲状态，而如果值为 1.0，则表示在所观察的最近一段时间内所有 CPU 都一直在主动运行。|
>| process.cpu.usage | 应用 CPU 使用率百分比 | 百分比 | Java 虚拟机进程最近的 CPU 使用率。 此值为 [0.0,1.0] 区间中的一个双精度值。 如果值为 0.0，则表示在所观察的最近一段时间内，任何 CPU 都未运行 JVM 进程中的线程，而如果值为 1.0，则表示在所观察的最近一段时间内，所有 CPU 都一直在主动运行 JVM 中的线程。 JVM 中的线程包括应用程序线程以及 JVM 内部线程。|
