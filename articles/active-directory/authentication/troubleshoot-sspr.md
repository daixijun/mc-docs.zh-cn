...
| 密码不符合模糊策略 | 使用的密码显示在禁用密码列表中，无法使用。 用户必须定义满足或超过禁用密码列表策略要求的密码。 |
