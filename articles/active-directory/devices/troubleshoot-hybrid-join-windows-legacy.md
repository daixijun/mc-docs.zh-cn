...
- 对于具有联合域的客户，如果服务连接点 (SCP) 配置为指向托管域名（例如 contoso.partner.onmschina.cn 而非 contoso.com），则下层 Windows 设备的混合 Azure AD 加入不会工作。
...
- AD FS 或 Azure AD 配置不当或网络存在问题
   ...
   - 你的组织使用 Azure AD 无缝单一登录，设备的 IE intranet 设置中不存在 `https://autologon.microsoftazuread-sso.com` 或 `https://aadg.chinacloudapi.cn.nsatc.net`，未对 Intranet 区域启用“允许通过脚本更新状态栏”。
- 登录身份不是域用户
