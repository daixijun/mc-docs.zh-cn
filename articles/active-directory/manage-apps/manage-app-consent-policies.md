...
1. （可选）添加“排除”条件集。
   ...
   $azureApi = Get-AzureADServicePrincipal -Filter "servicePrincipalNames/any(n:n eq 'https://management.chinacloudapi.cn/')"
...
| ClientApplicationsFromVerifiedPublisherOnly | 设置为 `$true` 将仅匹配具有已验证发布者的客户端应用程序。 设置为 `$false` 将匹配任何客户端应用，即使它没有已验证的发布者。 默认值为 `$false`。 |
