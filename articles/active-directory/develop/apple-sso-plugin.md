...
在这种情况下，应用程序创建网络请求并打开 Web 浏览器以让用户登录时，系统将提供 SSO。 用户重定向到 Azure AD 登录 URL 时，SSO 插件将验证 URL 并检查该 URL 是否有可用的 SSO 凭据。 如果有可用的 SSO 凭据，则 SSO 插件会将 SSO 凭据传递到 Azure AD，这将授权应用程序完成网络请求，而无需要求用户输入其凭据。 
